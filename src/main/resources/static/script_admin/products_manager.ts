				...
				if (data) {
					...
					for (let i = 0; i < data.length; ++i) {
						...
						searchOrderSuggesstionContainer.innerHTML +=
							...
							<span class="search-order-cusPhone">${bagCate['bagCateID']}</span>
							<span class="search-order-orderDate">${bagCate['name']}</span>
							<span class="search-order-orderDate">${bagCate['brand']}</span>
						</a>`
					}
				}
			})
